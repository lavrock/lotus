...
func TestPaymentChannels(t *testing.T) {
	...
	waitForHeight(ctx, paymentReceiver, chState.SettlingAt)
...
func newMockCLI(t *testing.T) *mockCLI {
	// Create a CLI App with an --api flag
	app := cli.NewApp()
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			Name:   "api",
...
func waitForHeight(ctx context.Context, paymentReceiver test.TestNode, height abi.ChainEpoch) {
	atHeight := make(chan struct{})
	chainEvents := events.NewEvents(ctx, paymentReceiver)
	chainEvents.ChainAt(func(ctx context.Context, ts *types.TipSet, curH abi.ChainEpoch) error {
		close(atHeight)
		return nil
	}, nil, 1, height)
...
func getPaychState(ctx context.Context, t *testing.T, paymentReceiver test.TestNode, chAddr address.Address) paych.State {
	act, err := paymentReceiver.StateGetActor(ctx, chAddr, types.EmptyTSK)
	require.NoError(t, err)

	store := cbor.NewCborStore(apibstore.NewAPIBlockstore(paymentReceiver))
